...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  */
  ...
  int n_lidar = 2;
  MatrixXd Zsig_pred = MatrixXd(n_lidar,2*n_aug_+1);
  ...
  //we can take a shortcut by reusing Xsig_pred (and weights_) instead of calculating sigma points again for the measurement
  //the result will be pretty close
  ...
  for(int i=0; i<2*n_aug_+1; i++)
  {
      ...
      Zcov_pred += weights_cov_(i) * z_diff * z_diff.transpose();
      ...
      T += weights_cov_(i) * x_diff * z_diff.transpose();
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  */
  ...
  int n_radar = 3;
  MatrixXd Zsig_pred = MatrixXd(n_radar,2*n_aug_+1);
