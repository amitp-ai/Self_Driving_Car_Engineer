...
def augment_flip(image):
    ...
    image1 = cv2.warpAffine(image, M_flip, (cols,rows))
    return image1
...
def generate_data_generator(batch_size=128):
    ...
    prob_areas_driving_log = pd.read_csv('training_data/Prob_Areas/shuffled_driving_log.csv', header=None)
    prob_areas_images_prepend_path = 'training_data/Prob_Areas/IMG/'
    prob_areas_total_size = prob_areas_driving_log.shape[0]
    ...
    prob_areas_num_images = math.ceil(batch_size*0.1) + 1 #so as to have a minimum size of 2 and not 1
    ...
    prob_areas_full_camera_data = {}
    prob_areas_full_images_shape = images_shape
    prob_areas_full_num_images = prob_areas_num_images*3*4 #three cameras worth of data and (original, brightness adjust, translate adjust, and flip) for each
    prob_areas_full_images_shape[0] = prob_areas_full_num_images
    prob_areas_full_camera_data['features'] = np.zeros(prob_areas_full_images_shape)
    prob_areas_full_camera_data['steering_angle'] = np.zeros(prob_areas_full_num_images)
    
    final_full_camera_data={}
    ...
    while True:
        if (i + num_images) >= total_size:
            i = 0
        if (prob_areas_i + prob_areas_num_images) >= prob_areas_total_size:
            prob_areas_i = 0
        ...
        for j in range(prob_areas_num_images):
            #Read center camera image
            image_file_name = prob_areas_images_prepend_path + prob_areas_driving_log.ix[prob_areas_i][0][9:]
            image_center = mpimg.imread(image_file_name) #.astype(np.uint8)
            image_center = cv2.resize(image_center, new_image_dim, interpolation = cv2.INTER_AREA)
            center_steer_angle = prob_areas_driving_log.ix[prob_areas_i][3]
            
            #Read left camera image
            image_file_name = prob_areas_images_prepend_path + prob_areas_driving_log.ix[prob_areas_i][1][10:]
            image_left = mpimg.imread(image_file_name) #.astype(np.uint8)
            image_left = cv2.resize(image_left, new_image_dim, interpolation = cv2.INTER_AREA)
            left_steer_angle = center_steer_angle + 0.25 #left camera
            
            #Read right camera image
            image_file_name = prob_areas_images_prepend_path + prob_areas_driving_log.ix[prob_areas_i][2][10:]       
            image_right = mpimg.imread(image_file_name) #.astype(np.uint8)
            image_right = cv2.resize(image_right, new_image_dim, interpolation = cv2.INTER_AREA)
            right_steer_angle = center_steer_angle - 0.25 #right camera
                    
            #Center camera image
            prob_areas_full_camera_data['features'][0*prob_areas_num_images+j] = image_center
            prob_areas_full_camera_data['steering_angle'][0*prob_areas_num_images+j] = center_steer_angle
            #augment brightness
            image_bright = augment_brigtness(image_center)
            prob_areas_full_camera_data['features'][1*prob_areas_num_images+j] = image_bright
            prob_areas_full_camera_data['steering_angle'][1*prob_areas_num_images+j] = center_steer_angle
            #augment translate
            image_translate,steer_translate = augment_translate(image_center)
            prob_areas_full_camera_data['features'][2*prob_areas_num_images+j] = image_translate
            prob_areas_full_camera_data['steering_angle'][2*prob_areas_num_images+j] = center_steer_angle + steer_translate
            #augment flip
            image_flip = augment_flip(image_center)
            prob_areas_full_camera_data['features'][3*prob_areas_num_images+j] = image_flip
            prob_areas_full_camera_data['steering_angle'][3*prob_areas_num_images+j] = -1*center_steer_angle
            
            #left camera image
            prob_areas_full_camera_data['features'][4*prob_areas_num_images + j] = image_left
            prob_areas_full_camera_data['steering_angle'][4*prob_areas_num_images + j] = left_steer_angle
            #augment brightness
            image_bright = augment_brigtness(image_left)
            prob_areas_full_camera_data['features'][5*prob_areas_num_images+j] = image_bright
            prob_areas_full_camera_data['steering_angle'][5*prob_areas_num_images+j] = left_steer_angle
            #augment translate
            image_translate,steer_translate = augment_translate(image_left)
            prob_areas_full_camera_data['features'][6*prob_areas_num_images+j] = image_translate
            prob_areas_full_camera_data['steering_angle'][6*prob_areas_num_images+j] = left_steer_angle + steer_translate
            #augment flip
            image_flip = augment_flip(image_left)
            prob_areas_full_camera_data['features'][7*prob_areas_num_images+j] = image_flip
            prob_areas_full_camera_data['steering_angle'][7*prob_areas_num_images+j] = -1*left_steer_angle           
        
            #Right camera image
            prob_areas_full_camera_data['features'][8*prob_areas_num_images + j] = image_right
            prob_areas_full_camera_data['steering_angle'][8*prob_areas_num_images + j] = right_steer_angle
            #augment brightness
            image_bright = augment_brigtness(image_right)
            prob_areas_full_camera_data['features'][9*prob_areas_num_images+j] = image_bright
            prob_areas_full_camera_data['steering_angle'][9*prob_areas_num_images+j] = right_steer_angle
            #augment translate
            image_translate,steer_translate = augment_translate(image_right)
            prob_areas_full_camera_data['features'][10*prob_areas_num_images+j] = image_translate
            prob_areas_full_camera_data['steering_angle'][10*prob_areas_num_images+j] = right_steer_angle + steer_translate
            #augment flip
            image_flip = augment_flip(image_right)
            prob_areas_full_camera_data['features'][11*prob_areas_num_images+j] = image_flip
            prob_areas_full_camera_data['steering_angle'][11*prob_areas_num_images+j] = -1*right_steer_angle
            
            prob_areas_i += 1
        
        final_full_camera_data['features'] = np.concatenate((full_camera_data['features'],prob_areas_full_camera_data['features']),axis=0)
        final_full_camera_data['steering_angle'] = np.concatenate((full_camera_data['steering_angle'],prob_areas_full_camera_data['steering_angle']),axis=0)
        #To randomly shuffle the training set
        features_train, _, steer_train, _ = train_test_split(final_full_camera_data['features'], 
                                                             final_full_camera_data['steering_angle'], test_size=1e-6, random_state=0)
        #print(12*num_images,'\n')
