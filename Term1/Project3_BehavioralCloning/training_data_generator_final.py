...
def shuffle_driving_log():
    ...
    val_array = shuffled_array[0:256]
    train_array = shuffled_array[256:]
    
    driving_log_temp = driving_log.copy()
    val_log = pd.DataFrame(driving_log.ix[0:255]) #just to initialize #with dataframe 0 and 255 are both inclusive
    train_log = pd.DataFrame(driving_log_temp.ix[0:(driving_log.shape[0]-256-1)]) #just to initialize
    
    for idx,val in enumerate(val_array):
        val_log.ix[idx] = driving_log.ix[val]

    for idx,val in enumerate(train_array):
        train_log.ix[idx] = driving_log_temp.ix[val] #needs to be another variable otherwise it affects val_log also
        
    print(val_log.ix[10][0])
    print(val_log.shape[0])

    print(train_log.ix[10][0])
    print(train_log.shape[0])
    
    print(val_log.center[0] in train_log.center.values)
    
    
    val_log.to_csv('training_data/Udacity_Data/validation_driving_log.csv', sep=',', header=False, index=False) #comma separated
    train_log.to_csv('training_data/Udacity_Data/shuffled_driving_log.csv', sep=',', header=False, index=False) #comma separated
...
def augment_translate(image, st_angle):
    ...
    #y_tr = 0
    #x_tr = 0
    y_tr = (np.random.uniform()-0.5)*2*rows*0.1 #translate up/down to emulate the effect of driving up/down on a road. max translate is 10% of the image height
    x_tr = (np.random.uniform()-0.5)*2*cols*0.2 #translate left/right to emulate the effect of having different camera mounts. max translate is 20% of the image width
    ...
    steer_delta = 1*x_tr/(cols*0.2)*0.2 #technically needs to be -1 but +1 seems to work better
...
def generate_data_generator(batch_size=16,straight_steer_discard_prob_th=0):
    ...
    full_num_images = num_images*4 #original and augmented image (for center and left/right)
    ...
    while True:
        ...
        for j in range(num_images):
            #To discard straight steering angled datapoints with a given probability
            a_1 = True
            while a_1:
                steer_angle = driving_log.ix[i][3]
                if abs(steer_angle)<0.1:
                    kp_st_steer = np.random.rand() #value between 0 and 1 [0,1)
                    if kp_st_steer >= straight_steer_discard_prob_th:
                        a_1 = False
                    else:
                        a_1=True #This is redundant. But added for readability
                        i+=1
                        if i >= total_size: i=0
                else:
                    a_1=False
            ...
            #Add original left/right camera image
            full_camera_data['features'][2*num_images+j] = image
            full_camera_data['steering_angle'][2*num_images+j] = steer_angle
            
            #Augment left/right camera image
            image, steer_angle = augment_brigtness(image, steer_angle)
            image, steer_angle = augment_translate(image, steer_angle)
            img_flip = np.random.randint(2)
            if img_flip == 1: #else don't flip
                image, steer_angle = augment_flip(image, steer_angle)
            #Add augmented left/right camera image                    
            full_camera_data['features'][3*num_images+j] = image
            full_camera_data['steering_angle'][3*num_images+j] = steer_angle
        ...
        #print(12*num_images,'\n')
        #steer_train[steer_train > 0.2] = 1.5 * steer_train[steer_train > 0.2] #increase the steering angle for images with high steering angles
        yield (features_train, steer_train) #one less data point than expected due to validation data generated above (minimum=1)

def generate_validation_data():
    val_log = pd.read_csv('training_data/Udacity_Data/validation_driving_log.csv', header=None)
    images_prepend_path = 'training_data/Udacity_Data/'
    image_file_name = images_prepend_path + val_log.ix[0][0]
    image_center = mpimg.imread(image_file_name)
    
    crop_dim = (int(image_center.shape[0]*0.25),int(image_center.shape[0]*0.9))
    new_image_dim = (160,80)
    
    num_images = val_log.shape[0]
    images_shape = [num_images,new_image_dim[1],new_image_dim[0],3]
    
    full_camera_data = {}
    full_camera_data['features'] = np.zeros(images_shape)
    full_camera_data['steering_angle'] = np.zeros(images_shape[0])
    
    for i in range(num_images):
        #Read center camera image
        image_file_name = images_prepend_path + val_log.ix[i][0]
        image = mpimg.imread(image_file_name) #.astype(np.uint8)
        image = image[crop_dim[0]:crop_dim[1]][:][:] #crop the horizon and car bumper out
        image = cv2.resize(image, new_image_dim, interpolation = cv2.INTER_AREA)
        steer_angle = val_log.ix[i][3]
        #Add validation center image
        full_camera_data['features'][i] = image
        full_camera_data['steering_angle'][i] = steer_angle
    
    return full_camera_data['features'], full_camera_data['steering_angle']
