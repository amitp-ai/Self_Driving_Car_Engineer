...
def test_train_nn(train_nn):
    ...
    def get_batches_fn(batach_size_parm):
        ...
        yield np.arange(np.prod(shape)).reshape(shape) #changed by Amit from return to yield
...
def test_for_kitti_dataset(data_dir):
    ...
    assert training_images_count == 279, 'Expected 289 training images, found {} images.'.format(training_images_count)
    assert training_labels_count == 279, 'Expected 289 training labels, found {} labels.'.format(training_labels_count)
