...
# This is based off of: https://github.com/balancap/SSD-Tensorflow/blob/master/nets/ssd_vgg_512.py and Amit's version of ssd_300
...
class SSDNet(object):
   ...
   The default image size used to train this network is 512x512.
    ...
    default_params = SSDParams(
        ...
        feat_shapes=[(64, 64), (32, 32), (16, 16), (8, 8), (4, 4), (2, 2), (1, 1)], #output feature size
        anchor_size_bounds=[0.10, 0.90],
        anchor_sizes=[(20.48, 51.2), #This is used after being normalized to image_shape in the anchor_box generation function
                      (51.2, 133.12), #Also, each number in anchor_sizes is scale i.e. sqrt(w*h)
                      (133.12, 215.04),
                      (215.04, 296.96),
                      (296.96, 378.88),
                      (378.88, 460.8),
        ...
        anchor_ratios=[[2, .5], #TODO: need to verify this
                       ...
                       [2, .5],
                       [2, .5]],
        anchor_steps=[8, 16, 32, 64, 128, 256, 512], #This is same as effective stride. But not sure of blocks 10 and 11
        anchor_offset=0.5, #TODO: need to verify this
        normalizations= [20, -1, -1, -1, -1, -1, -1], #As per the paper, only normalize the output of conv_4 and not the feature maps
...
def ssd_net(inputs,
    ...
    with tf.variable_scope(scope, 'ssd_512_vgg', [inputs], reuse=reuse):
        ...
        #added dropout. not present in balancap github for ssd512
        net = tf.layers.dropout(net, rate=dropout_keep_prob, training=is_training) #rate=dropout rate (no dropout in ssd512)
        # Block 7: 1x1 conv.
        ...
        with tf.variable_scope(end_point):
            ...
            net = custom_layers.pad2d(net, pad=(1, 1))
            net = slim.conv2d(net, 256, [3, 3], stride=2, scope='conv3x3', padding='VALID')
        end_points[end_point] = net #block10: effective stride=128
        end_point = 'block11'
        with tf.variable_scope(end_point):
            net = slim.conv2d(net, 128, [1, 1], scope='conv1x1')
            net = custom_layers.pad2d(net, pad=(1, 1))
        ...
        end_points[end_point] = net #block11: effective stride=256
        end_point = 'block12'
        with tf.variable_scope(end_point):
            net = slim.conv2d(net, 128, [1, 1], scope='conv1x1')
            #print(net.get_shape().as_list())
            ...
            #print(net.get_shape().as_list())
        end_points[end_point] = net #block11: effective stride=512 (due to effect of padding and 4x4 filter)
...
ssd_net.default_image_size = 512 #FIXME: I dont' know where this is used.
