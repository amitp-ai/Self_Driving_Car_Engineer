...
def optimum_policy2D_DP(grid,init,goal,cost):
...
strt = time.time()
pth_len, pol_2d = optimum_policy2D_DP(grid,init,goal,cost)
stp = time.time()
print("Using DP: ", stp-strt, "seconds")
...
def optimum_policy2D_UCS(grid,init,goal,cost):
    # ----------------------------------------
    ...
    #find the path
    actions3D = [[[-1 for col in range(len(grid[0]))] for row in range(len(grid))],
    ...
    while not found and not resign:
        if len(fringe) == 0:
            ...
            fringe.reverse()
            next = fringe.pop() #pop the point with smallest cost
            row = next[1]
            col = next[2]
            orient = next[3]
            g = next[0]
...
strt = time.time()
pth_len, pol_2d,act_3d = optimum_policy2D_UCS(grid,init,goal,cost)
stp = time.time()
print("\nUsing UCS: ", stp-strt, "seconds")
print("Path Length is: ", pth_len)
print()
for i in range(len(pol_2d)):
    print(pol_2d[i])
